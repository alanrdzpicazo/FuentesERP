...
namespace ProbeMedic.Controles
{
    public class MEPDataGridView : DataGridView
    {
        ...
        public MEPDataGridView()
        {
            ...
            this.vAllowFocusReadOnlyColumns = true;
            this.vEnterMoveNextColumn = true;
        }
        ...
        public bool EnterMoveNextColumn
        {
            ...
            set { vEnterMoveNextColumn = value; }
        }
        ...
        {
            if (this.NeedPaintEmptyArea && this.FillEmptyArea)
                this.Invalidate(true);
            if (!this.AllowFocusReadOnlyColumns)
                this.OnGoToEnableColumn();
        }
        [PermissionSet(SecurityAction.Demand, Name = "FullTrust")]
        private void WmLButtonDblClk(ref Message m)
        {
            this.OnPreProcessMouse(ref m);
        }
        [PermissionSet(SecurityAction.Demand, Name = "FullTrust")]
        ...
        {
            this.OnPreProcessKeyboard(ref m);
        }
        ...
        [PermissionSet(SecurityAction.Demand, Name = "FullTrust")]
        private void OnPreProcessMouse(ref Message m)
        {
            bool preProcessMouseMessage = true;
            if (!this.AllowFocusReadOnlyColumns)
            {
                Point point = this.PointToClient(Control.MousePosition);
                DataGridView.HitTestInfo hitInfo = this.HitTest(point.X, point.Y);
                if (hitInfo.Type == DataGridViewHitTestType.Cell)
                    preProcessMouseMessage = !this.Columns[hitInfo.ColumnIndex].ReadOnly;
            }

            if (preProcessMouseMessage)
                base.WndProc(ref m);
        }
        [PermissionSet(SecurityAction.Demand, Name = "FullTrust")]
        private void OnPreProcessKeyboard(ref Message m)
        {
            int wParam = m.WParam.ToInt32();
            bool preProcessKeyboardMessage = true;

            if (!this.AllowFocusReadOnlyColumns)
            {
                bool IsShiftPress = NativeMethods.GetKeyState(NativeMethods.VK_SHIFT) < 0;
                if (wParam == NativeMethods.VK_LEFT || (wParam == NativeMethods.VK_TAB && IsShiftPress))
                {
                    this.OnGoToPreviewColumn();
                    preProcessKeyboardMessage = false;
                }
                else if ((wParam == NativeMethods.VK_RETURN && EnterMoveNextColumn) || wParam == NativeMethods.VK_RIGHT || wParam == NativeMethods.VK_TAB)
                {
                    this.OnGoToNextColumn();
                    preProcessKeyboardMessage = false;
                }
            }
            else if (wParam == NativeMethods.VK_RETURN && this.EnterMoveNextColumn)
            {
                this.OnGoToNextEnableColumn();
                preProcessKeyboardMessage = false;
            }

            if (preProcessKeyboardMessage)
                base.WndProc(ref m);
        }
        private void OnGoToEnableColumn()
        {
            for (int c = this.CurrentCell.ColumnIndex; c < this.Columns.Count; c++)
            {
                if (!this.Columns[c].ReadOnly)
                {
                    this.CurrentCell = this.CurrentRow.Cells[c];
                    break;
                }
            }
        }
        ...
        {
            if (this.CurrentCell.ColumnIndex == this.Columns.Count)
                return;
        ...
        private void OnGoToNextEnableColumn()
        {
            if (this.CurrentCell.ColumnIndex + 1 == this.Columns.Count)
                return;

            this.CurrentCell = this.CurrentRow.Cells[this.CurrentCell.ColumnIndex + 1];
        }
        private void OnGoToPreviewColumn()
        {
            if (this.CurrentCell.ColumnIndex == 0)
                return;

            for (int i = this.CurrentCell.ColumnIndex - 1; i >= 0; i--)
            {
                if (!this.Columns[i].ReadOnly)
                {
                    this.CurrentCell = this.CurrentRow.Cells[i];
