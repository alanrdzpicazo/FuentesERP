...
{
    public partial class FRM_ZONIFICACION_FPENFERMERIA_ASEG : FormaBase
    {
        SQLCatalogos catalogosSQL = new SQLCatalogos();
        Funciones fx = new Funciones();

        DataTable datos = new DataTable();
        int K_Precio_ForAsegu_Enfermeria = 0, K_Pais = 0, K_Estado = 0, K_Ciudad = 0, K_Oficina = 0, K_Precio_Enfermeria = 0, K_Aseguradora = 0;
        ...
        public FRM_ZONIFICACION_FPENFERMERIA_ASEG()
        {
            InitializeComponent();
        }
        ...
        public void LLenarGrid(int K_Pais, Int32 K_Estado, Int32 K_Ciudad, Int32 K_Precio_Enfermeria, int K_Oficina, DateTime FechaInicial, DateTime FechaFinal, int K_Aseguradora)
        {
            datos = catalogosSQL.Sk_Zonificacion_FPEnfermeria_Aseg(Convert.ToInt16(K_Pais), K_Estado, K_Ciudad, K_Precio_Enfermeria, K_Oficina, FechaInicial, FechaFinal, K_Aseguradora);
            msg = string.Empty;
            ...
            if (datos != null)
            {
                ...
                grdDatos.Columns["K_Precio_ForAsegu_Enfermeria"].HeaderText = "Clave";
        ...
        public override void BaseBotonGuardarClick()
        {
            ...
            try
            {
                res = catalogosSQL.In_Zonificacion_ForAsegu_Precios_Enfermeria(ref CampoIdentity,Convert.ToInt16(zonFE_Oficina1.K_Pais), Convert.ToInt32(zonFE_Oficina1.K_Estado), Convert.ToInt16(zonFE_Oficina1.K_Ciudad), Convert.ToInt32(K_Precio_Enfermeria), Convert.ToInt16(zonFE_Oficina1.K_Oficina), Convert.ToDecimal(txtPrecio.Text), FechaInicial, FechaFinal, Convert.ToInt16(ucAseguradora1.K_Aseguradora), ref msg);
            }
            catch (Exception ex)
            {
        ...
        public override void BaseBotonBuscarClick()
        {
            ...
            K_Pais = zonFE_Oficina1.K_Pais;
            K_Ciudad = zonFE_Oficina1.K_Ciudad;
            K_Estado = zonFE_Oficina1.K_Estado;
            K_Oficina = zonFE_Oficina1.K_Oficina;
            K_Precio_Enfermeria = Convert.ToInt32(K_Precio_Enfermeria);
            K_Aseguradora = Convert.ToInt16(ucAseguradora1.K_Aseguradora);

            LLenarGrid(K_Pais, K_Estado, K_Ciudad, K_Precio_Enfermeria, K_Oficina, FechaInicial, FechaFinal, K_Aseguradora);
        ...
        private void FRM_ZONIFICACION_FPENFERMERIA_ASEG_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            zonFE_Oficina1.txt_G_Pais.Text = string.Empty;
            zonFE_Oficina1.txt_G_Estado.Text = string.Empty;
            zonFE_Oficina1.txt_G_Ciudad.Text = string.Empty;
            zonFE_Oficina1.txt_G_Oficina.Text = string.Empty;
        ...
        public override void BaseBotonCancelarClick()
        {
            ...
            BaseMetodoLimpiaControles();
            ucAseguradora1.txt_Z_Aseguradora.Text = string.Empty;
            zonFE_Oficina1.Focus();
        }
        public override void BaseBotonModificarClick()
        {

            BaseBotonGuardar.Enabled = false;
            K_Precio_ForAsegu_Enfermeria = int.Parse(grdDatos.CurrentRow.Cells["K_Precio_ForAsegu_Enfermeria"].Value.ToString());
            ...
            FRM_ACTUALIZA_ZFP_ENFERMERIA_ASEG actualiza = new FRM_ACTUALIZA_ZFP_ENFERMERIA_ASEG(K_Precio_ForAsegu_Enfermeria, K_Pais, K_Ciudad, D_Ciudad, K_Estado, K_Oficina, K_Precio_Enfermeria, Convert.ToDecimal(grdDatos.CurrentRow.Cells["Precio"].Value.ToString()), FechaInicial, FechaFinal,K_Aseguradora);
        ...
        public override bool BaseFuncionValidaciones()
        {
            ...
            if (zonFE_Oficina1.txt_G_Pais.Text.Trim().Length == 0)
            {
                MessageBox.Show("FALTA CAPTURAR PAIS...!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                zonFE_Oficina1.txt_G_Pais.Focus();
                return false;
            }
            if (zonFE_Oficina1.txt_G_Estado.Text.Trim().Length == 0)
            {
                MessageBox.Show("FALTA CAPTURAR ESTADO..!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                zonFE_Oficina1.txt_G_Estado.Focus();
                return false;
            }
            if (zonFE_Oficina1.txt_G_Ciudad.Text.Trim().Length == 0)
            {
                MessageBox.Show("FALTA CAPTURAR CIUDAD...!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                zonFE_Oficina1.txt_G_Ciudad.Focus();
                return false;
            }
            if (zonFE_Oficina1.txt_G_Oficina.Text.Trim().Length == 0)
            {
